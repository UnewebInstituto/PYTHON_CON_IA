...
with st.sidebar:
    ...
    if st.button("Procesar documentos", type="primary", disabled=not archivos):
        with st.spinner(
            "Cargando, fragmentando e indexando documentos... "
            "(la primera vez puede tardar varios minutos: se descarga "
            "el modelo de embeddings, ~470 MB. Las siguientes veces será "
            "instantáneo porque queda en caché local)."
        ):
